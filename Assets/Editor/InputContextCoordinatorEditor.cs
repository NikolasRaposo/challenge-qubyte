...
[CustomEditor(typeof(Managers.InputContextCoordinator))]
public class InputContextCoordinatorEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        // Inspector padrão (compatível com multi-seleção)
        Editor.DrawPropertiesExcluding(serializedObject, new string[] { "m_Script" });

        EditorGUILayout.Space(6);
        showActivators = EditorGUILayout.BeginFoldoutHeaderGroup(showActivators, "Ativadores do Coordenador de Contexto (UnityEvents)");
        if (showActivators)
        {
            EnsureLinkStyle();

            var coordinator = (Managers.InputContextCoordinator)target;
            if (_activators == null)
            {
                if (GUILayout.Button("Varrer UnityEvents na cena", GUILayout.Height(22)))
                {
                    _activators = ScanForActivators(coordinator);
                }
                EditorGUILayout.HelpBox("Procura UnityEvents na cena que chamam métodos públicos do InputContextCoordinator (ex.: SetUiContext, SetPlayerContext, SetBlockInputContext, EnableUiInteractions, DisableUiInteractions).", MessageType.Info);
            }
            else
            {
                if (_activators.Count == 0)
                {
                    EditorGUILayout.HelpBox("Nenhum ativador encontrado. Configure componentes com UnityEvents apontando para métodos do InputContextCoordinator quando necessário.", MessageType.Warning);
                }
                else
                {
        ...
        if (showDeepScan)
        {
            EditorGUILayout.HelpBox("Procura chamadas em código para métodos do Coordenador via Singleton (InputContextCoordinator.Instance.*).", MessageType.Info);
            if (GUILayout.Button("Varrer chamadas em código", GUILayout.Height(22)))
            {
                _deepResults = DeepCallScanner.ScanSingletonCalls(
                    typeName: nameof(Managers.InputContextCoordinator),
                    methodNames: new[] {
                        nameof(Managers.InputContextCoordinator.SetUiContext),
                        nameof(Managers.InputContextCoordinator.SetPlayerContext),
                        nameof(Managers.InputContextCoordinator.SetBlockInputContext),
                        nameof(Managers.InputContextCoordinator.EnableUiInteractions),
                        nameof(Managers.InputContextCoordinator.DisableUiInteractions)
                    }
                );
            }
    ...
    private System.Collections.Generic.List<ActivatorEntry> ScanForActivators(Managers.InputContextCoordinator coordinator)
    {
    ...
    private static void TryCollectActivator(UnityEngine.Events.UnityEventBase ev, MonoBehaviour mb, System.Type compType, string fieldName, Managers.InputContextCoordinator coordinator, System.Collections.Generic.List<ActivatorEntry> acc)
    {
        if (ev == null) return;
        int count = 0;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (targetObj == coordinator && !string.IsNullOrEmpty(method))
            {
