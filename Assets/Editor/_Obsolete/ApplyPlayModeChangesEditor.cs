...
public class ApplyPlayModeChangesEditor : EditorWindow
{
    ...
    void OnGUI()
    {
        GUILayout.Label("Ferramenta de Aplica��o de Mudan�as do Play Mode", EditorStyles.boldLabel);

        changesData = (PlayModeChangesData)EditorGUILayout.ObjectField("Dados de Mudan�as", changesData, typeof(PlayModeChangesData), false);

        if (GUILayout.Button("Aplicar Mudan�as na Cena"))
        {
            if (changesData == null)
            {
        ...
        if (GUILayout.Button("Limpar Dados de Mudan�as"))
        {
            if (changesData != null)
            {
                ...
                Debug.Log("Dados de mudan�as limpos.");
            }
        }
    }
    ...
    void ApplyChangesToScene()
    {
        ...
        {
            Debug.LogError("N�o � poss�vel aplicar mudan�as no modo Play. Saia do Play Mode primeiro.");
            return;
        }
        ...
        {
            Debug.LogError("A cena atual n�o foi salva ou n�o tem um caminho v�lido. Salve a cena antes de tentar aplicar as mudan�as.");
            return;
        }
        ...
        {
            Debug.LogWarning($"N�o h� mudan�as salvas para a cena atual ('{currentScene.name}'). As mudan�as salvas s�o para outra cena. Limpe os dados do Play Mode se n�o forem relevantes.");
            return;
        }
        else if (changesData.objectsToSave.Count == 0)
        {
            Debug.Log("N�o h� mudan�as salvas para aplicar.");
            return;
        }

        Debug.Log($"Aplicando mudan�as salvas do Play Mode para a cena: {currentScene.name}...");
        ...
        {
            Debug.LogWarning("GameObject PlayModeTracker n�o encontrado na cena. Criando um novo para aplicar as mudan�as.");
        ...
        {
            Debug.LogError("Falha ao obter/criar o GameObject PlayModeTracker. N�o foi poss�vel aplicar as mudan�as.");
            return;
        }
        ...
        {
            GameObject processedObject = null; // Refer�ncia ao objeto que ser� criado ou atualizado

            if (data.isNewObject)
            {
                // Criar um novo objeto (inst�ncia de prefab)
                if (!string.IsNullOrEmpty(data.prefabPath))
                {
                    ...
                    if (prefab != null)
                    {
                        ...
                        {
                            // Encontrado por GUID (se o novo objeto j� tinha um GUID salvo, por exemplo, por ter vindo de um prefab que j� tem UniqueIdComponent)
                            Debug.LogWarning($"Objeto {data.instanceIdAtPlayMode} (GUID: {data.objectGuid}) j� existe na cena como filho do PlayModeTracker. Atualizando sua transforma��o em vez de criar um novo.");
                            processedObject = existingTempObj;
                        }
                        else if (existingObjectsByName.TryGetValue(data.instanceIdAtPlayMode, out existingTempObj))
                        {
                            // Fallback por nome
                            Debug.LogWarning($"Objeto {data.instanceIdAtPlayMode} j� existe na cena como filho do PlayModeTracker (pelo nome). Atualizando sua transforma��o em vez de criar um novo.");
                            processedObject = existingTempObj;
                        }
                    ...
                    {
                        Debug.LogError($"Prefab n�o encontrado no caminho: {data.prefabPath}. N�o foi poss�vel recriar o objeto '{data.instanceIdAtPlayMode}'.");
                    }
                }
                else
                {
                    Debug.LogWarning($"Objeto '{data.instanceIdAtPlayMode}' marcado como novo, mas sem caminho de prefab. N�o ser� recriado.");
                }
            }
            else // Objeto EXISTENTE que foi movido/modificado
            {
                ...
                {
                    Debug.LogWarning($"Objeto existente com ID/nome '{data.instanceIdAtPlayMode}' (GUID: '{data.objectGuid}') n�o encontrado na hierarquia do PlayModeTracker. Pode ter sido deletado ou renomeado no Editor. Ignorando.");
                }
            }

            // --- NOVO: L�gica para adicionar/garantir UniqueIdComponent ap�s a aplica��o da mudan�a ---
            if (processedObject != null)
            {
                UniqueIdComponent objUniqueId = processedObject.GetComponent<UniqueIdComponent>();
                if (objUniqueId == null)
                {
                    // Adiciona o componente usando Undo para que a a��o possa ser desfeita (Ctrl+Z)
                    objUniqueId = Undo.AddComponent<UniqueIdComponent>(processedObject);
                    // O GUID ser� gerado automaticamente pelo Awake/OnValidate do UniqueIdComponent
                ...
                {
                    // Caso raro: componente existe mas GUID n�o foi gerado (talvez erro anterior). Garante que ele gere.
                    // Isso � tratado internamente pelo UniqueIdComponent via OnValidate/Awake, mas um SetDirty ajuda.
        ...
        Debug.Log("Mudan�as aplicadas e cena salva!");
        changesData.ClearData();
        EditorUtility.SetDirty(changesData);
        AssetDatabase.SaveAssets();
    }
    ...
    private static void MapChildrenRecursive(Transform parent, Dictionary<string, GameObject> byGuid, Dictionary<string, GameObject> byName)
    {
        foreach (Transform child in parent)
        {
            ...
            if (uniqueId != null && !string.IsNullOrEmpty(uniqueId.Guid))
            {
                ...
                {
                    Debug.LogWarning($"GUID duplicado encontrado na hierarquia do PlayModeTracker: '{uniqueId.Guid}' para objeto '{child.name}'. Apenas a primeira inst�ncia ser� considerada para atualiza��o por GUID.");
                }
            }
            ...
            {
                Debug.LogWarning($"Nome de objeto duplicado encontrado na hierarquia do PlayModeTracker: '{child.name}'. Apenas a primeira inst�ncia ser� considerada para atualiza��o por nome.");
            }
