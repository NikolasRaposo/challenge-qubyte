...
public class SplinePathGamePlayController1 : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (!IsActive())
        {
            Debug.Log($"[SplinePath1] Ignorado (delay {activationDelay:F2}s ativo). Collider: '{other.name}' no {Time.time - _enabledTime:F2}s", this);
            return;
        }
        ...
        Debug.Log($"[SplinePath1] Entrada: '{saci.name}' parent -> '{(animateHolder != null ? animateHolder.name : name)}' pos -> { (animateHolder != null ? animateHolder.position : transform.position) }", this);
        ...
        if (splineAnimate != null)
        {
            ...
            splineAnimate.Play();
            Debug.Log($"[SplinePath1] Iniciando animação na spline '{splineAnimate.name}'", this);
        }
    }
    ...
    public void HandleEndTriggerEnter(Collider other)
    {
        ...
        Debug.Log($"[SplinePath1] Encerrado: '{saci.name}' restaurado para parent '{(_originalParent != null ? _originalParent.name : "<null>")}'", this);
