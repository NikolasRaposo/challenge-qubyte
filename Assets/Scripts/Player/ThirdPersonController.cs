    ...
    #region Fields
        ...
        private void JumpAndGravity()
        {
            if (Grounded)
            {
                ...
                if (InputManager.Instance.Jump && _jumpTimeoutDelta <= 0.0f && _verticalVelocity <= 0.0f)
                {
            ...
            else // Se não estamos no chão...
            {
                ...
                if (InputManager.Instance.Jump && _airJumpsRemaining > 0 && _isDoubleJumpEnabled && Time.time >= _lastJumpTime + MinJumpInterval)
                {
                    _airJumpsRemaining--;
        ...
        private void Move() {
            ...
            if (Grounded)
            {
                ...
                {
                    _targetRotation = Mathf.Atan2(InputManager.Instance.Move.x, InputManager.Instance.Move.y) * Mathf.Rad2Deg + _mainCamera.transform.eulerAngles.y;
        ...
        private void CameraRotation()
        {
            ...
            if (InputManager.Instance.Look.sqrMagnitude >= threshold && !_lockCameraPosition)
            {
                float deltaTimeMultiplier = IsCurrentDeviceMouse ? 1.0f : Time.deltaTime;
                _cinemachineTargetYaw += InputManager.Instance.Look.x * deltaTimeMultiplier;
                _cinemachineTargetPitch += InputManager.Instance.Look.y * deltaTimeMultiplier;
            }
